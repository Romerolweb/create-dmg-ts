...
/**
 * Escapes special characters in text for RTF format
 * @param text - The plain text to escape
 * @returns RTF-escaped text
 */
function getRtfUnicodeEscapedString(text: string): string {
	let result = '';
...
/**
 * Wraps plain text in RTF format structure
 * @param text - The plain text to wrap
 * @returns RTF formatted text with hex encoding
 */
function wrapInRtf(text: string): string {
...
/**
 * Serializes a hex string into formatted hex chunks for Rez format
 * @param text - The hex string to serialize
 * @returns Formatted hex string for Rez file
 */
function serializeString(text: string): string {
	...
	return '\t$"' + chunks.map(x => {
		const subChunks = x.match(/.{1,4}/g);
		return subChunks ? subChunks.join(' ') : x;
	}).join('"\n\t$"') + '"';
}

/**
 * Adds a Software License Agreement to the DMG if license files are present
 * Looks for license.txt, license.rtf, or sla.r in the current working directory
 * 
 * @param dmgPath - Path to the DMG file
 * @param dmgFormat - Format of the DMG (e.g., 'ULFO')
 */
export default async function addLicenseAgreementIfNeeded(
	dmgPath: string,
	dmgFormat: string
): Promise<void> {
	// Valid SLA filenames
	...
	if (hasRaw) {
		...
		if (hasRtf) {
			...
			const result = await execa('/usr/bin/textutil', ['-convert', 'txt', '-stdout', rtfSlaFile]);
